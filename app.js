...
const rmDefault = bot.keyboard([
    ...
    [bot.button("✅Chegirmalar"),bot.button("👨‍💻Iqtidorli o'quvchilar")]
],{resize: true});
//
...
bot.on('text', async (msg)=>{
    ...
    else if(tx == "👨‍💻Iqtidorli o'quvchilar"){
        jsonfile.readFile(path.join(__dirname, '/','iqtidor.json'),(err, obj)=>{
            if (err) console.error(err)
            var data = ""
            for(var i = 0; i<obj.length; i++){
                data += `*🎉${i+1} - O'rin:*\n👨‍💻Ismi: *${obj[i].ism}*\n🌐Yo'nalishi: *${obj[i].yonalish}*\n\n`
            }
            bot.sendMessage(cid, data, {parseMode: "markdown"})
          })
    }
